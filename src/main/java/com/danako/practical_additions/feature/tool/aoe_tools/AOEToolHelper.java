package com.danako.practical_additions.feature.tool.aoe_tools;
...
public final class AOEToolHelper {
    ...
    public static int getRadius(HasAOEToolType type) {
        return type == HasAOEToolType.SICKLE_5X5 ? 2 : 1;
    }
    ...
    public static List<BlockPos> getTargetPositions(Level level, HasAOEToolType type, BlockPos center, Direction face) {
        return getAreaPositions(center, face, getRadius(type));
    }


    public static boolean isAreaEffectAllowed(Player player, HasAOEToolType type, BlockState primaryState) {
        if (player != null && player.isShiftKeyDown()) return false;
        return isValidTarget(type, primaryState);
    }

    public static boolean isValidTarget(HasAOEToolType type, BlockState state) {
    ...
    public static void breakArea(ServerPlayer player, Level level, BlockPos primaryPos, BlockState primaryState, HasAOEToolType type, Direction face) {
    ...
    public static boolean harvestAndReplant(ServerPlayer player, Level level, BlockPos center, Direction face) {
        ...
        for (BlockPos pos : withCenter(center, face, getRadius(HasAOEToolType.SICKLE_5X5))) {
